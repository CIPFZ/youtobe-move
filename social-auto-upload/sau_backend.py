...
from hk_puller import sync_hk_videos, run_hk_poller, fetch_hk_stats as _fetch_hk_stats, mark_uploaded
from conf import HK_AUTO_DOWNLOAD
...
def hk_list_videos():
    """Get HK video list from local DB."""
...
def hk_mark_uploaded():
    ...
    ok = mark_uploaded(video_id, platform, account)
    return jsonify({
        "video_id": video_id,
        "uploaded": ok,
        "platform": platform,
...
def hk_trigger_sync():
    """Trigger a manual sync from HK server in background."""
    def _bg():
        try:
            summary = sync_hk_videos()
            logger.info("Manual HK sync complete: %s", summary)
        except Exception as exc:
            logger.error("Manual HK sync failed: %s", exc)

    t = threading.Thread(target=_bg, daemon=True)
    t.start()
    return jsonify({"started": True, "message": "HK sync triggered in background"}), 200


@app.route('/hk/stats', methods=['GET'])
def hk_stats():
    """Return HK sync statistics."""
    conn = _hk_conn()
    ...
    last_sync = conn.execute(
        "SELECT synced_at, new_count, downloaded_count, error FROM hk_sync_log ORDER BY id DESC LIMIT 1"
    ).fetchone()
    conn.close()
    return jsonify({
        "total_videos": total,
...
@app.route('/hk/file/<video_id>', methods=['GET'])
def hk_serve_file(video_id: str):
    """Serve a downloaded video file."""
    conn = _hk_conn()
    row = conn.execute(
        "SELECT file_path, video_id FROM hk_videos WHERE video_id=? AND download_status='downloaded'",
        (video_id,),
    ).fetchone()
    conn.close()
    if not row:
        return jsonify({"error": "File not found", "video_id": video_id}), 404
    file_dir = Path(row["file_path"])
    candidates = sorted(file_dir.glob(f"{video_id}.*"))
    # prefer mp4
    mp4_files = [f for f in candidates if f.suffix == '.mp4']
    target = mp4_files[0] if mp4_files else (candidates[0] if candidates else None)
    if not target or not target.exists():
        return jsonify({"error": "File missing on disk", "video_id": video_id}), 404
...
if __name__ == '__main__':
    ...
    if HK_AUTO_DOWNLOAD:
        try:
            run_hk_poller()
            print("HK poller started in background")
        except Exception as exc:
            print(f"HK poller failed to start: {exc}")
